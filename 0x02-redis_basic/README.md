...
### **What does `web.py` do?**  
It implements a **caching layer** around fetching a web page: (1) **`get_page(url)`** — fetches the URL with `requests.get` and returns the response text; (2) a **decorator** (e.g. `wrap_requests`) that (a) **increments** a Redis key `count:{url}` every time the URL is requested, (b) **returns the cached HTML** from Redis key `cached:{url}` if it exists, (c) otherwise calls the real `get_page`, **stores the result** in Redis with a TTL (e.g. 10 seconds) using `setex`, and returns it. So the first request hits the web; later requests within the TTL come from Redis.  
**Run:** Start Redis (`redis-server`), then e.g. `python3 -c "from web import get_page; print(get_page('http://example.com')[:200])"`. Call again to see cache and count in action.

### **What does `exercise.py` do?**  
It contains **Redis exercises** (get, set, incr, etc.) so you can practice basic Redis operations from Python (e.g. connect, set a key, get it, increment a counter).  
**Run:** `python3 exercise.py` if it has a `if __name__ == "__main__"` block, or run snippets in the REPL.

### **What does `0-main.py` do?**  
It is a **demo or test script** that uses the Redis client or the caching logic (e.g. calls `get_page` or exercises Redis) so you can verify everything works.  
**Run:** `python3 0-main.py` (with Redis server running).
